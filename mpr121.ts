...
namespace smartfeldSensoren {
    ...
    export class MPR121 {
        ...
        // weight=70
        initTouchController(): void {

            this.MPR121_ADDRESS = 0x5B;
            this.mpr121_ADD = 0x5B;
            ...
            const addr = this.MPR121_ADDRESS
            mpr121.reset(addr)
            ...
            control.inBackground(() => this.detectAndNotifyTouchEvents());
        }
        ...
        let previousTouchStatus = 0
            ...
            while (true) {
                const touchStatus = mpr121.readTouchStatus(this.MPR121_ADDRESS)
                this.touchController.lastTouchStatus = touchStatus
                ...
                for (let touchSensorBit = 1; touchSensorBit <= 2048; touchSensorBit <<= 1) {
                    ...
                    if ((touchSensorBit & touchStatus) !== 0) {
                        if (!((touchSensorBit & previousTouchStatus) !== 0)) {
                            ...
                            this.touchController.lastEventValue = touchSensorBit
                        }
                    }
        ...
        */
        ...
        onTouchSensorTouched(sensor: TouchSensor, handler: () => void) {
            this.initTouchController()
            control.onEvent(MPR121_TOUCH_SENSOR_TOUCHED_ID, sensor, () => {
                this.setupContextAndNotify(handler)
            })
        }
        ...
        */
        //% sensor.fieldEditor="gridpicker" sensor.fieldOptions.columns=3
        //% sensor.fieldOptions.tooltips="false"
        onTouchSensorReleased(sensor: TouchSensor, handler: () => void) {
            this.initTouchController()
        ...
        */
        onAnyTouchSensorTouched(handler: () => void) {
            this.initTouchController()
        ...
        */
        ...
        onAnyTouchSensorReleased(handler: () => void) {
            this.initTouchController()
        ...
        setupContextAndNotify(handler: () => void) {
            this.touchController.lastEventValue = control.eventValue()
            handler()
        }
        ...
        touchSensor(): number {
            this.initTouchController()
            if (this.touchController.lastEventValue !== 0) {
                return this.getSensorIndexFromSensorBitField(this.touchController.lastEventValue)
            } else {
                return 0
            }
        }

        getSensorIndexFromSensorBitField(touchSensorBit: TouchSensor) {
        ...
        //% sensor.fieldEditor="gridpicker" sensor.fieldOptions.columns=3
        //% sensor.fieldOptions.tooltips="false"
        isTouched(sensor: TouchSensor): boolean {
            this.initTouchController()
            return (this.touchController.lastTouchStatus & sensor) !== 0
        }
    }
